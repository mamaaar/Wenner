...
class WennerView extends Ui.View { // Vue qui affiche l'heure
	...
	/***********Heure des messages d�finit en absolu***********/ 
    ...
    */
    ...
    /**********************************************************/
    var tabMessages;	// Pour r�cup le tableau des messages selon la condition
    
	var today;			// var pour r�cup l'heure, la minute et la seconde courantes
	var timer; 			// Timer
    ...
    function initialize() {
        ...
        // R�cup de la condition + du tableau correspondant *****************
	...
	function callback() { // Permet d'actualiser l'interface toute les secondes
       	...
       	var today = System.getClockTime(); // r�cup�re l'heure et la minute courante
       	...
       	if (sec == 0){
				...
				&& messageSortieMinute.toNumber()==today.min.toNumber()) {
	       		...
	       		if (1==today.hour.toNumber() && 1==today.min.toNumber()) {
	       		 ...
	       		 // ----- Ajout �criture dans le fichier de backup ----- //
				...
				// ----- Fin ajout �criture dans le fichier de backup ----- //
		
				// ----- Surveillance de la batterie ----- //
				// Si on atteint 5% de batteries, peu importe l'heure on fais une sauvegarde de tout ce qu'on a actuellement
				var myStats = System.getSystemStats();
		
				if(myStats.battery <= minBattery && !saveLowBattery){ // si la batterie est <= 5% et qu'on a pas d�j� sauvegarder pour la batterie 
    ...
    function afficherMessage(groupe, type){
		...
		var tabKeys = groupe.keys(); //Tableau contenant les cl�s du groupe
    	var messageCode = tabKeys[random]; // R�cup�re une cl� au hassard
    	var messageId = groupe.get(messageCode); // R�cup�re le message de la cl�
